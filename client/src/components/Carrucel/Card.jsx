...
export default function Card({ id, name, price, image, stock, brand }) {
  ...
  const handleFavorites = (id) => {
    if (Object.keys(user).length === 0) {
      ...
      WishListService.PostProductWishList(data)
        .then((response) => {
          return Swal.fire({
            icon: "success",
            title: "all good!",
  ...
  return (
    <div class="w-full max-w-xl bg-white border border-gray-400 rounded-lg dark:bg-gray-900 dark:border-gray-700 transform transition duration-500 hover:scale-105 mt-3 mb-3">
      <a href="#">
        <img
          class="rounded-t-lg w-full h-[300px] text-center px-20"
          src={image}
          alt="product image"
      ...
      <div class="px-3 pb-1">
        <div className="flex flex-row justify-between">
          ...
          <BsFillHeartFill
            ...
            className="w-5 h-5 mt-3 cursor-pointer text-slate-900 dark:text-slate-100"
          />
        </div>
        ...
        <div class="flex items-center justify-between">
          <span class="text-3xl font-bold text-gray-900 mb-2 dark:text-white">
            ${price}
          </span>
          <a
            className="text-white cursor-pointer mb-1 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300
