      ...
      <div>
       <Link to={'/Store'}  className="px-2 flex mt-4 ml-4 bg-gray-300 text-black  rounded-xl border-2 border-white text-xl text-center hover:bg-transparent hover:text-white">back to store</Link>
        ...
        <div className="justify-center flex-1 flex flex-col py-10 px-0 sm:px-8 lg:px-20 sm:py-9 md:py-9  xl:px-24 ">
            ...
            <h1 class='mt-6 text-3xl font-extrabold m-4' >{details.name}</h1>
          
            <span className= 'mt-6 text-2xl font-extrabold m-4'>Price: ${details.price}</span>
            {/* <span className="mt-6 text-2xl font-bold ">Stock: {details.stock}</span> */}
            <p className="my-6 text 2x1 leading-loose font-bold m-4">Description: {details.description}</p>
